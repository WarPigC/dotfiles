...
var GifRenderer = class {
  ...
  addVideoFrame(frame) {
    ...
    this.addFrameToGif(frame.duration / 1e3);
  }
  addFrameToGif(delay) {
...
// plugins/GifCaptioner/src/render/image.ts
async function captionImage(url, width, height, transform, mime) {
  const progress = new ProgressDisplay("Rendering GIF", "Fetching");
  try {
    const res = await BdApi.Net.fetch(url);
    const arrayBuffer = await res.arrayBuffer();
